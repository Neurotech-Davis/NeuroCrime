...
expName = 'Psychopy Draft'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/tonysaldana/Downloads/Psychopy Draft.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "FirstVid" ---
    Question = visual.TextStim(win=win, name='Question',
        text='Who Scored the point during this clip.',
        font='Arial',
    ...
    movie = visual.MovieStim(
        win, name='movie',
        filename='/Users/tonysaldana/Downloads/#2 Cooper Flagg (Duke Blue Devils)-selected/Cooper-Flag-Clip.mp4', movieLib='ffpyplayer',
    ...
    Prompt = visual.TextStim(win=win, name='Prompt',
        text="Press the key 'f' if they scored the point\n\nPress the key 'j' if they didn't score the point.",
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    image = visual.ImageStim(
        win=win,
        name='image', 
        image=img_file, mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.5, 1.0),
        color=[0.8824, 0.9451, 1.0000], colorSpace='rgb', opacity=None,
    ...
    # --- Prepare to start Routine "FirstVid" ---
    # create an object to store info about Routine FirstVid
    FirstVid = data.Routine(
        name='FirstVid',
        components=[Question, movie, Prompt],
    )
    FirstVid.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for FirstVid
    FirstVid.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    FirstVid.tStart = globalClock.getTime(format='float')
    FirstVid.status = STARTED
    thisExp.addData('FirstVid.started', FirstVid.tStart)
    FirstVid.maxDuration = None
    # keep track of which components have finished
    FirstVidComponents = FirstVid.components
    for thisComponent in FirstVid.components:
    ...
    # --- Run Routine "FirstVid" ---
    FirstVid.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 33.0:
        ...
        if movie.status == NOT_STARTED and tThisFlip >= 10.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if movie.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > movie.tStartRefresh + 13.0-frameTolerance or movie.isFinished:
                # keep track of stop time/frame for later
        ...
        if Prompt.status == NOT_STARTED and tThisFlip >= 23.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # if Prompt is stopping this frame...
        if Prompt.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Prompt.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
                Prompt.tStop = t  # not accounting for scr refresh
                Prompt.tStopRefresh = tThisFlipGlobal  # on global time
                Prompt.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Prompt.stopped')
                # update status
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            FirstVid.forceEnded = routineForceEnded = True
            break
    ...
    # --- Ending Routine "FirstVid" ---
    for thisComponent in FirstVid.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for FirstVid
    FirstVid.tStop = globalClock.getTime(format='float')
    FirstVid.tStopRefresh = tThisFlipGlobal
    thisExp.addData('FirstVid.stopped', FirstVid.tStop)
    movie.stop()  # ensure movie has stopped at end of Routine
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if FirstVid.maxDurationReached:
        routineTimer.addTime(-FirstVid.maxDuration)
    elif FirstVid.forceEnded:
        routineTimer.reset()
    else:
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('Downloads/PsychoPy.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if Testing.maxDurationReached:
            routineTimer.addTime(-Testing.maxDuration)
        elif Testing.forceEnded:
            routineTimer.reset()
        else:
