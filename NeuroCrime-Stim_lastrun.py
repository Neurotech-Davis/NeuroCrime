    ...
    for thisNew_clip in new_clip:
        ...
        while continueRoutine and routineTimer.getTime() < 23.0:
            ...
            if clips.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > clips.tStartRefresh + 20-frameTolerance or clips.isFinished:
                    # keep track of stop time/frame for later
        ...
        for thisTrial_2 in trials_2:
            ...
            for thisNew_face in new_face:
                ...
                while continueRoutine and routineTimer.getTime() < 7.0:
                    ...
                    if mugshots.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > mugshots.tStartRefresh + 4-frameTolerance:
                            # keep track of stop time/frame for later
