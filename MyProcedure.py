...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\diego\\OneDrive\\Documents\\Coding\\Neurotech\\NeuroCrime\\MyProcedure.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    explaining_task_text = visual.TextStim(win=win, name='explaining_task_text',
        ...
        font='Arial',
        pos=(0, 0), draggable=False, height=75.0, wrapWidth=1500.0, ori=0.0, 
    ...
    grey_dot = visual.ImageStim(
        win=win,
        name='grey_dot', 
        image='Display/black_dot-removebg-preview.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=None,
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Press f or j to continue to the next clip',
        font='Arial',
        pos=(0, 0), draggable=False, height=75.0, wrapWidth=1500.0, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    while continueRoutine and routineTimer.getTime() < 33.0:
        ...
        if explaining_task_text.status == STARTED:
            ...
            if tThisFlipGlobal > explaining_task_text.tStartRefresh + 10-frameTolerance:
        ...
        if explaining_text_2.status == NOT_STARTED and tThisFlip >= 12-frameTolerance:
            # keep track of start time/frame for later
        ...
        if explaining_text_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > explaining_text_2.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if button_clarification_text.status == NOT_STARTED and tThisFlip >= 23-frameTolerance:
            # keep track of start time/frame for later
        ...
        if button_clarification_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > button_clarification_text.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
    ...
    new_clip = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('stim-vids.xlsx'), 
        seed=None, 
    )
    ...
    for thisNew_clip in new_clip:
        ...
        showing_movie.maxDuration = length + 1
        # keep track of which components have finished
        ...
        while continueRoutine:
            ...
            if tThisFlip > showing_movie.maxDuration-frameTolerance:
                showing_movie.maxDurationReached = True
                continueRoutine = False
            ...
            if clips.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > clips.tStartRefresh + 22-frameTolerance or clips.isFinished:
                    # keep track of stop time/frame for later
        ...
        # the Routine "showing_movie" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        for thisTrial_2 in trials_2:
            ...
            for thisNew_face in new_face:
                ...
                while continueRoutine and routineTimer.getTime() < 6.5:
                    ...
                    if mugshots.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > mugshots.tStartRefresh + 3.5-frameTolerance:
                            # keep track of stop time/frame for later
                    ...
                    if grey_dot.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > grey_dot.tStartRefresh + 1.5-frameTolerance:
                            # keep track of stop time/frame for later
